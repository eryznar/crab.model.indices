...
rkc_kgkm <- haul_rkc %>%
  ...
  # retain weight only for males that are included in the model
  mutate(male.wt = case_when(
    SEX == 1 & WIDTH_MM >= 64 ~ CALCULATED_WEIGHT,
    TRUE ~ 0
  )) %>%
  # make sampling factor column for males included in the model only; for all others sampling factor = 0
  mutate(sampling.factor = case_when(
    SEX == 1 & WIDTH_MM >= 64 ~ SAMPLING_FACTOR,
    TRUE ~ 0
  )) %>%
  mutate(wt.kg = (male.wt * sampling.factor) / 1000) %>%
...
grid_new.rkc <- expand.grid(LONGITUDE = seq(from= min(rkc_kgkm$LONGITUDE), to= max(rkc_kgkm$LONGITUDE), by=0.05),
...
grid_yrs.rkc <- replicate_df(grid_new.rkc, "SURVEY_YEAR", unique(rkc_kgkm$SURVEY_YEAR))
dplyr::glimpse(grid_yrs.rkc)

# predictions on new data
predictions.nsrkc <- predict(m.nsrkc, newdata = grid_yrs.rkc, return_tmb_object = T)
...
map.fe.re.rkc <- plot_map(predictions.nsrkc$data, exp(est)) +
  scale_fill_viridis_c(trans = "sqrt") + #sqrt so we can see changes better?
  ggtitle("Prediction (fixed effects + all random effects)")

map.fe.re.yrs.rkc <- plot_map(predictions.nsrkc$data %>% filter(SURVEY_YEAR > 2003), exp(est)) +
  scale_fill_viridis_c(trans = "sqrt") + #sqrt so we can see changes better?
  ggtitle("Prediction (fixed effects + all random effects)")

ggsave(file.path(plotdir, "map_fixedeff_randomeff_nsrkc.png"), plot = map.fe.re.rkc, height = 4.2, width = 7, units = "in")

# just look at fixef
plot_map(predictions.nsrkc$data, exp(est_non_rf)) +
...
plot_map(predictions.nsrkc$data, omega_s) +
...
plot_map(predictions.nsrkc$data, epsilon_st) +
  ggtitle("Spatiotemporal random effects only") +
  scale_fill_gradient2()

View(predictions.nsrkc)
...
index.nsrkc <- get_index(predictions.nsrkc, area = 1, bias_correct = TRUE)

# index values in metric tons rather than kg
index.nsrkc.t <- index.nsrkc %>% mutate(est.t = est / 1000, lwr.t = lwr/1000, upr.t = upr/1000)

# plot index
ggplot(index.nsrkc.t, aes(SURVEY_YEAR, est.t)) + geom_line() +
  geom_ribbon(aes(ymin = lwr.t, ymax = upr.t), alpha = 0.4) +
  xlab('Year') + ylab('Biomass estimate (t)') +
  scale_y_continuous(label=scales::comma)

# biomass estimates
mutate(index.nsrkc.t, cv = sqrt(exp(se^2) - 1)) %>% 
  select(-log_est, -se) %>%
  knitr::kable(format = "pandoc", digits = c(0, 0, 0, 0, 0, 0, 0, 2))
