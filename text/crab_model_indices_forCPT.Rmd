...
We fit models using the R package *sdmTMB*. After fitting each model, we used the following steps to evaluate models:

- the sdmTMB::sanity() function. Output of this function for a model that passes all sanity checks looks like this:
...
- If a model passed all the sanity checks, we used the R package *DHARMa* (Hartig 2022) to calculate the DHARMa residuals using the function DHARMa::dharma_residuals().
